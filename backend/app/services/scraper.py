...
class ScraperConfig:
    ...
    JOB_DETAIL_BASE_URL = "https://www.linkedin.com/jobs/view/"
    JOBS_PER_PAGE = 25
    MIN_DELAY = 2
    MAX_DELAY = 5
    RATE_LIMIT_DELAY = 30
    ...
    HEADERS = {
        ...
        "Accept-Language": "en-US,en;q=0.5,vi-VN,vi;q=0.9",
...
class LinkedInJobsScraper:
    ...
    def _extract_job_id(self, job_card: BeautifulSoup) -> Optional[str]:
        try:
            return job_card.get("data-entity-urn").split(":")[-1]
        except Exception:
            return None

    def _fetch_job_description(self, job_id: str) -> str:
        try:
            url = f"{ScraperConfig.JOB_DETAIL_BASE_URL}{job_id}"
            ...
            print(f"Failed to fetch job description for job ID {job_id}: {str(e)}")
            return "N/A"
    ...
    def _extract_job_data(self, job_card: BeautifulSoup) -> Optional[JobData]:
        try:
            ...
            job_id = self._extract_job_id(job_card)
            description = self._fetch_job_description(job_id) if job_id else "N/A"
    ...
    def save_results_csv(self, jobs: List[JobData], filename: str = "linkedin_jobs.csv") -> None:
        if not jobs:
            return
        with open(filename, "w", newline="", encoding="utf-8") as csvfile:
            fieldnames = ["title", "company", "location", "job_link", "posted_date", "description"]
